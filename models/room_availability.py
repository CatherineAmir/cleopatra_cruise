...
class CruiseRoomAvailability(models.Model):
    ...
    available_for_booking=fields.Boolean(string='Available For Booking',default=True,compute='_compute_available_rooms',store=True,readonly=False,tracking=True)
    ...
    def _compute_available_rooms(self):
        for record in self:
            ...
            if record.available_rooms:
                record.available_for_booking = True
            else:
                record.available_for_booking = False